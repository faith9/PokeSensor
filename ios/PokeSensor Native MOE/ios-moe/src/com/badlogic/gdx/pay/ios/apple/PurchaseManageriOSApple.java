...
public class PurchaseManageriOSApple implements PurchaseManager, SKPaymentTransactionObserver {
    ...
    private class IosFetchProductsAndInstallDelegate implements SKProductsRequestDelegate,
        ...
        public void productsRequestDidReceiveResponse(SKProductsRequest request,
            ...
            ArrayList<SKPaymentTransaction> copiedTransactions = new ArrayList<>();

            for (SKPaymentTransaction transaction : transactions) {
                copiedTransactions.add(transaction);
            }

            for (SKPaymentTransaction transaction : copiedTransactions) {
                defaultQueue.finishTransaction(transaction);
            }

            log(LOGTYPELOG, "Manually finished " + copiedTransactions.size() + " transactions so they don't trip us up later");
            if (true) return;
            ...
            if (transactions.size() > 0) {
                ...
                log(LOGTYPELOG, "No transactions to complete! Won't even try in that case.");
            }
        }
