...
public class TwoCaptchaService {
	...
	public String solveCaptcha() throws InterruptedException, IOException {
		...
		MapController.features.print("Captcha:", "Raw captcha Id html: " + hw.getHtml());
		...
		if (firstResponse.toLowerCase().contains("error") || firstResponse.toLowerCase().contains("ip_banned")) {
			...
			MapController.features.print("PokeFinder", error);
			MapController.features.longMessage(error);
			return "";
		}

		String captchaId = firstResponse.replaceAll("\\D", "").replaceAll("OK\\|","");
		MapController.features.print("Captcha", "Captcha ID: " + captchaId);
		int timeCounter = 0;
		...
		MapController.features.print("Captcha", "Raw solved response: " + hw.getHtml());
