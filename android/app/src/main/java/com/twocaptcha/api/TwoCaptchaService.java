...
public class TwoCaptchaService {
	...
	public String solveCaptcha() throws InterruptedException, IOException {
		...
		PokeFinderActivity.features.print("Captcha:", "Raw captcha Id html: " + hw.getHtml());
		String firstResponse = hw.getHtml();

		if (firstResponse.toLowerCase().contains("error") || firstResponse.toLowerCase().contains("ip_banned")) {
			String error = "2Captcha error:\n" + firstResponse.replaceAll("\\D","");
			PokeFinderActivity.features.print("PokeFinder", error);
			PokeFinderActivity.features.longMessage(error);
			return "";
		}

		String captchaId = firstResponse.replaceAll("\\D", "").replaceAll("OK\\|","");
		PokeFinderActivity.features.print("Captcha", "Captcha ID: " + captchaId);
		int timeCounter = 0;
		...
		String secondResponse = hw.getHtml();

		if (secondResponse.toLowerCase().contains("error")) {
			String error = "2Captcha error:\n" + secondResponse.replaceAll("\\n","");
			PokeFinderActivity.features.print("PokeFinder", error);
			PokeFinderActivity.features.longMessage(error);
			return "";
		}
		...
		PokeFinderActivity.features.print("Captcha", "Raw solved response: " + hw.getHtml());
		String gRecaptchaResponse = secondResponse.replaceAll("OK\\|", "").replaceAll("\\n", "");
